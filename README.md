...
You can find settings in `Preferences -> Other Settings -> Limited WIP`, where you can:
 ...
 - enable/disable displaying timer in auto-revert widget. 
   ...
   In other cases, you might prefer not to see time left and just focus on making the smallest change possible.
...
like resetting the timer after each commit and that IntelliJ will be asking on each revert from terminal 
if you really want to load changes from file system).

Some time later (in 2015) after using auto-revert for a while on large legacy code bases, 
I found that it can be too harsh sometimes, especially when you're in exploration mode.
