...
data class ChangeSize(val value: Int, val isApproximate: Boolean = false) {
    operator fun plus(that: ChangeSize) =
        ChangeSize(value + that.value, isApproximate or that.isApproximate)
...
data class ChangeSizesWithPath(val value: List<Pair<String, ChangeSize>>) {
    val totalChangeSize =
        value.map { it.second }.fold(ChangeSize.empty) { acc, it -> acc + it }
